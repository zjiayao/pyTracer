...
from pytracer.core.definition cimport FLOAT_t, INT_t, UINT_t, EPS, fsqrt
from pytracer import FLOAT
from pytracer.geometry.geometry cimport _Arr3
from pytracer.data.spectral import CIE_Y_INTEGRAL
import numpy as np
...
cdef INT_t SAMPLED_LAMBDA_START = 400
cdef INT_t SAMPLED_LAMBDA_END = 700
cdef INT_t N_SPECTRAL_SAMPLES = 30


cpdef FLOAT_t[:] xyz2rgb(FLOAT_t[:] xyz):
	return np.array([3.240479 * xyz[0] - 1.537150 * xyz[1] - 0.498535 * xyz[2],
	                 -0.969256 * xyz[0] + 1.875991 * xyz[1] + 0.041556 * xyz[2],
	                 0.055648 * xyz[0] - 0.204043 * xyz[1] + 1.057311 * xyz[2]], dtype=FLOAT)


cpdef FLOAT_t[:] rgb2xyz(FLOAT_t[:] rgb):
	return np.array([0.412453 * rgb[0] + 0.357580 * rgb[1] + 0.180423 * rgb[2],
	                 0.212671 * rgb[0] + 0.715160 * rgb[1] + 0.072169 * rgb[2],
	                 0.019334 * rgb[0] + 0.119193 * rgb[1] + 0.950227 * rgb[2]], dtype=FLOAT)


# Spectrum Definitions
cpdef enum SpectrumType:
	REFLECTANCE = 0
	ILLUMINANT = 1


cdef class CoefficientSpectrum:
	"""
	...
	def __cinit__(self, INT_t n_samples=3, FLOAT_t v=0.):
		self.c = np.full(n_samples, v, dtype=FLOAT)
	...
	@staticmethod
	cdef inline CoefficientSpectrum create_from_array(FLOAT_t[:] arr):
		cdef CoefficientSpectrum c = CoefficientSpectrum(len(arr))
		c.arr.c[...] = arr
		return c
	...
	def create(cls, arg):
		...
		cdef CoefficientSpectrum ret = CoefficientSpectrum()

		if isinstance(arg, CoefficientSpectrum):
			arg = <CoefficientSpectrum> arg
			ret.c[...] = arg.c
			return ret
	...
	cdef inline bint is_black(self):
		"""
	...
	cpdef CoefficientSpectrum sqrt(self):
		"""
...
cdef class SampledSpectrum(CoefficientSpectrum):
	"""
	...
	def __cinit__(self, v=None):
		if v is None:
			self.c = np.full(N_SPECTRAL_SAMPLES, 0., dtype=FLOAT)
		else:
			self.c = v.copy()
	...
	def from_sampled(cls, lam: Iterable, v: Iterable):
		...
		self = cls()
		from pytracer.core.definition import lerp
		for i in range(N_SPECTRAL_SAMPLES):
